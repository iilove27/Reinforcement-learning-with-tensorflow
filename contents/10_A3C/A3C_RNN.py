...
class ACNet(object):
        ...
        if scope == GLOBAL_NET_SCOPE:   # get global network
            ...
            with tf.variable_scope(scope):
                ...
                with tf.name_scope('choose_a'):  # use local params to choose action
                    nd = normal_dist.sample(1)
                    sqz = tf.squeeze(nd, axis=[0, 1])
                    self.A = tf.clip_by_value(sqz, A_BOUND[0], A_BOUND[1])
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope('critic'):   # only critic controls the rnn update
            ...
            # lstm_cell = cudnn_rnn.CudnnLSTM(1, cell_size)
            # outputs, _ = lstm_cell(s)
